...
| `src/schema.ts` | Element allowlist (`elements`) + built-in host prop rules (`checkHostProp`: string `style`, `dangerouslySetInnerHTML` &c., non-variable `on*`, `javascript:` URLs) as shared canonical checks — parse-time events and render-time enforcement both call them. `formatPromptContract` serializes the schema for the generating model's system prompt. React-free. |
