...
| `onJsxError`         | `(event: JsxErrorEvent) => void`                    | —              | Unified structured JSX-level error events, fired at parse time.  |
...
`createParser` accepts `mismatchedTag`, `onError`, and `onJsxError` like the
React adapter. Since the core knows nothing about React components, pass
...
away. `onJsxError` is a unified channel for exactly that:
...
- **Fires in every recovery mode** — reporting is decoupled from recovery, so
  `mismatchedTag: "autoclose"` or `onUnknownComponent: "passthrough"` still
  produce events while the tree keeps degrading gracefully as before.
...
The existing `onError` callback is unchanged and keeps its current triggers;
`onJsxError` is the recommended channel for JSX-level errors.
